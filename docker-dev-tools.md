...
- [Development Environments & IDEs](#development-environments--ides) - VS Code Server, RStudio, Vert, Webtop, Node-RED, n8n
...
**Note:** The `bretfisher/jekyll-serve` image is particularly useful for repositories with complex dependencies. It auto-runs `bundle install` and starts the server with sensible defaults.
...
### Webtop
Full Linux desktop environment accessible via web browser (solves X11 issues).

```bash
# Ubuntu MATE desktop (recommended)
...
# Alpine with KDE (minimal size)
docker run -d --name=webtop -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:alpine-kde

# Arch Linux with KDE
docker run -d --name=webtop -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:arch-kde

# Fedora with XFCE
docker run -d --name=webtop -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:fedora-xfce

# With custom user/group IDs (Linux)
docker run -d --name=webtop -e PUID=1000 -e PGID=1000 -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:ubuntu-mate

# With custom timezone
docker run -d --name=webtop -e TZ=America/New_York -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:ubuntu-mate
...
```bash
# Linux/macOS
alias dtwebtop='docker run -d --name=webtop -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:ubuntu-mate'
alias dtwebtopxfce='docker run -d --name=webtop -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:ubuntu-xfce'
alias dtwebtopstart='docker start webtop'
alias dtwebtopstop='docker stop webtop'
alias dtwebtoplogs='docker logs webtop -f'

# PowerShell
function dtwebtop { docker run -d --name=webtop -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:ubuntu-mate }
function dtwebtopxfce { docker run -d --name=webtop -p 3000:3000 -p 3001:3001 -v webtop-config:/config --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/webtop:ubuntu-xfce }
function dtwebtopstart { docker start webtop }
function dtwebtopstop { docker stop webtop }
function dtwebtoplogs { docker logs webtop -f }
```

**Note:** Access Webtop at `http://localhost:3000`. This gives you a full Linux desktop in your browser - no X11 forwarding needed! Perfect for running GUI applications on Windows, macOS, or Linux. Port 3001 is for HTTPS (optional). Install any GUI app with `sudo apt install` (password: `abc`). Includes Firefox, terminal, file manager, and text editors pre-installed.

**Use Cases:**
- Run GUI Linux apps without X11 configuration
- Test desktop applications in isolated environment
- Access Linux tools from Windows/macOS via browser
- Remote development with visual tools
- Run occasional GUI apps without permanent installation
...
### MQTT Testing Tools
Test MQTT messages with command-line or native GUI tools.

```bash
# The mosquitto_sub and mosquitto_pub commands are already available (see above)
# These are the recommended way to test MQTT

# Subscribe to all topics
docker run --rm eclipse-mosquitto mosquitto_sub -h host.docker.internal -t "#" -v

# Subscribe to specific topic
docker run --rm eclipse-mosquitto mosquitto_sub -h host.docker.internal -t "home/temperature"

# Publish test message
docker run --rm eclipse-mosquitto mosquitto_pub -h host.docker.internal -t "test/topic" -m "Hello MQTT"

# Monitor all traffic (verbose)
docker run --rm eclipse-mosquitto mosquitto_sub -h host.docker.internal -t "$SYS/#" -v
```
...
**For GUI:** Download native MQTT Explorer from [mqtt-explorer.com](http://mqtt-explorer.com/) (Windows, macOS, Linux)
