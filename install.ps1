...
function Add-Functions {
    ...
    $functions += '# Docker Toolbox Manager Commands'
    $functions += ''
    $functions += '# List all dt- functions'
    $functions += 'function dt-list {'
    $functions += '    Write-Host "Installed Docker Toolbox functions:" -ForegroundColor Cyan'
    $functions += '    Write-Host ""'
    $functions += '    $dtFunctions = Get-Command -Name "dt-*" -CommandType Function -ErrorAction SilentlyContinue | Where-Object { $_.Source -eq '''' } | Sort-Object Name'
    $functions += '    if ($dtFunctions.Count -eq 0) {'
    $functions += '        Write-Host "  No Docker Toolbox functions found" -ForegroundColor Yellow'
    $functions += '        return'
    $functions += '    }'
    $functions += '    foreach ($func in $dtFunctions) { Write-Host "  $($func.Name)" -ForegroundColor Green }'
    $functions += '    Write-Host ""'
    $functions += '    Write-Host "Usage: <function-name> [args]" -ForegroundColor Gray'
    $functions += '}'
    $functions += ''
    $functions += '# Show info about a specific tool'
    $functions += 'function dt-info {'
    $functions += '    param([Parameter(Mandatory=$true)][string]$ToolName)'
    $functions += '    if (-not $ToolName.StartsWith("dt-")) { $ToolName = "dt-$ToolName" }'
    $functions += '    $func = Get-Command -Name $ToolName -CommandType Function -ErrorAction SilentlyContinue'
    $functions += '    if (-not $func) { Write-Host "Tool ' + [char]39 + '$ToolName' + [char]39 + ' not found. Run ' + [char]39 + 'dt-list' + [char]39 + ' to see available tools" -ForegroundColor Red; return }'
    $functions += '    Write-Host "Tool: $($func.Name)" -ForegroundColor Cyan'
    $functions += '    Write-Host ""; Write-Host "Definition:" -ForegroundColor White'
    $functions += '    Write-Host "  $($func.Definition)" -ForegroundColor Gray'
    $functions += '}'
    $functions += ''
    $functions += '# Search for tools'
    $functions += 'function dt-search {'
    $functions += '    param([Parameter(Mandatory=$true)][string]$Keyword)'
    $functions += '    Write-Host "Searching for ' + [char]39 + '$Keyword' + [char]39 + ' in Docker Toolbox functions:" -ForegroundColor Cyan'
    $functions += '    Write-Host ""'
    $functions += '    $dtFunctions = Get-Command -Name "dt-*" -CommandType Function -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "*$Keyword*" } | Sort-Object Name'
    $functions += '    if ($dtFunctions.Count -eq 0) { Write-Host "  No functions found" -ForegroundColor Yellow; return }'
    $functions += '    foreach ($func in $dtFunctions) { Write-Host "  $($func.Name)" -ForegroundColor Green }'
    $functions += '}'
    $functions += ''
    $functions += '# Show Docker images'
    $functions += 'function dt-images {'
    $functions += '    Write-Host "Docker images used by Docker Toolbox:" -ForegroundColor Cyan'
    $functions += '    Write-Host ""'
    $functions += '    $dtFunctions = Get-Command -Name "dt-*" -CommandType Function -ErrorAction SilentlyContinue | Where-Object { $_.Source -eq '''' }'
    $functions += '    if ($dtFunctions.Count -eq 0) { Write-Host "No Docker Toolbox functions found" -ForegroundColor Yellow; return }'
    $functions += '    $images = @(); foreach ($func in $dtFunctions) { if ($func.Definition -match ' + [char]39 + 'docker run[^"]*?([a-z0-9/_.-]+:[a-z0-9._-]+)' + [char]39 + ') { $images += $Matches[1] } }'
    $functions += '    $images = $images | Select-Object -Unique | Sort-Object'
    $functions += '    if ($images.Count -eq 0) { Write-Host "No Docker images found" -ForegroundColor Yellow; return }'
    $functions += '    Write-Host ("{0,-40} {1,-12} {2}" -f "Image", "Status", "Size") -ForegroundColor White'
    $functions += '    foreach ($image in $images) {'
    $functions += '        $imageInfo = docker images $image --format "{{.Repository}}:{{.Tag}}" 2>$null | Where-Object { $_ -eq $image }'
    $functions += '        if ($imageInfo) { $size = docker images $image --format "{{.Size}}" 2>$null; Write-Host ("{0,-40} {1,-12} {2}" -f $image, "Pulled", $size) -ForegroundColor Green }'
    $functions += '        else { Write-Host ("{0,-40} {1,-12} {2}" -f $image, "Not pulled", "-") -ForegroundColor Yellow }'
    $functions += '    }'
    $functions += '}'
    $functions += ''
    $functions += '# Show help'
    $functions += 'function dt-help {'
    $functions += '    Write-Host "Docker Toolbox Manager - Use dt-list, dt-info, dt-search, dt-images, or dt-help" -ForegroundColor Cyan'
    $functions += '    Write-Host "Documentation: https://github.com/yourusername/docker-toolbox" -ForegroundColor Gray'
    $functions += '}'
    $functions += ''
    $functions += '# End Docker Toolbox Manager Commands'
    $functions += ''
